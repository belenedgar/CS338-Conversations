...
def clear_everything(messages, channel_data, channel_id, last_message_time, lull_type):
    global promptSent
    messages.clear()
...
def run():
    ...
    async def inactivity_checker(channel,channel_id): 
        ...
        while True: 
            ...
            if promptSent == True:
                lull_type = 0
                clear_everything(messages, channel_data, channel_id, last_message_time, lull_type)
                # messages.clear()
                # for id in channel_data.keys():
...
#helper to get members in a channel TODO : NEED TO ENABLE Intents.members in bot permissions to get member list
    ...
    async def on_message(message):
        ...
        if message.channel.type != discord.ChannelType.private_thread:
            ...
            if lull_algorithm(message.content, buzzwords):
                lull_type -= 1
            ...
            if threshold > 4:
                ...
                #we should reset data here to be empty array again so we arent passing irrelevant messages to opena
                # Reset threshold and message count
                lull_type = 0
                # threshold = 0
                # m_count = 0
